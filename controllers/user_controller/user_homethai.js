...
const getUserThaiDay = (req, res) => {
    const userId = req.userId;
    db.query('SELECT * FROM user_thaiday WHERE user_id = ?', [userId], (err, results) => {
        if (err) return res.status(500).json({ error: 'Internal server error' });
        res.json(results);
    });
};

const addUserThaiDay = (req, res) => {
    const { date } = req.body;
    db.query('INSERT INTO user_thaiday (date, user_id) VALUES (?, ?)', [date, req.userId], (err, result) => {
        if (err) {
...
const deleteUserThaiDay = (req, res) => {
    const { id } = req.params;

    const deleteUserCodes = 'DELETE FROM user_thaicode WHERE entry_id IN (SELECT id FROM user_thaientrie WHERE day_id = ?)';
    const deleteUserEntries = 'DELETE FROM user_thaientrie WHERE day_id = ?';
    const deleteUserDays = 'DELETE FROM user_thaiday WHERE id = ? ';
...
const totalUserThaiPrice = (req, res) => {
    const { dayId } = req.params; // Make sure dayId is being received
